...
<body>
    ...
    <main class="page faq-page bg-dark">
                ...
                if ($data = json_decode(file_get_contents('../data/disaster.json'), true)) {
                    if (sizeof($data['disaster']) > 0) {
                        foreach ($data['disaster'] as $key => $disaster) {
                            if ($disaster['insurance'] == $_SESSION['company']) {
                                array_push($allDisaster, $disaster);
                            }
                        }
                        foreach ($allDisaster as $key => $disaster) {
                            echo "De : ".$disaster['insuredName']." ".$disaster['insuredFname'];
                            echo " | ".$disaster['vehicle'];
                            echo "<div>";
                                ...
                                echo "<div id='".$key."' style='display:none'>";
                                    echo '<p>Date : '.$disaster["disasterDate"].'</p>';
                                    echo '<p>Nombre de blessés : '.$disaster["nbOfInjured"].'</p>';
                                    echo '<p>Natures des dégats : '.$disaster["damageNature"].'</p>';
                                    echo '<p>Circonstances : '.$disaster["circonstances"].'</p>';
                                    echo '<p>Message : '.$disaster["message"].'</p>';
                                    if ($disaster['validation'] == 'notValidate') {
                                        $path = '../data/proofsDisaster/'.$disaster["vehicle"];
                                        $path2 = '../data/constats/'.$disaster["vehicle"];
                                        if (file_exists($path.'.png')) {
                                            echo '<p><a href="'.$path.'".png" download="preuve_du_sinistre.png">Télécharger la preuve du sinistre</a></p>';;
                                        } elseif (file_exists($path.'.jpg')) {
                                            echo '<p><a href="'.$path.'".jpg" download="preuve_du_sinistre.jpg">Télécharger la preuve du sinistre</a></p>';
                                        } elseif (file_exists($path.'.jpeg')) {
                                            echo '<p><a href="'.$path.'".jpeg" download="preuve_du_sinistre.jpeg">Télécharger la preuve du sinistre</a></p>';
                                        } elseif (file_exists($path.'.pdf')) {
                                            echo '<p><a href="'.$path.'".pdf" download="preuve_du_sinistre.pdf">Télécharger la preuve du sinistre</a></p>';
                                        }
                                        if (file_exists($path2.'.png')) {
                                            echo '<p><a href="'.$path2.'".png" download="constat.png">Télécharger le constat</a></p>';
                                        } elseif (file_exists($path2.'.jpg')) {
                                            echo '<p><a href="'.$path2.'".jpg" download="constat.jpg">Télécharger le constat</a></p>';
                                        } elseif (file_exists($path2.'.jpeg')) {
                                            echo '<p><a href="'.$path2.'".jpeg" download="constat.jpeg">Télécharger le constat</a></p>';
                                        } elseif (file_exists($path2.'.pdf')) {
                                            echo '<p><a href="'.$path2.'".pdf" download="constat.pdf">Télécharger le constat</a></p>';
                                        }
                                        echo (' <input class="button" type="button" name="validate" value="Valider" onclick="validateDisaster(\''.$disaster['insuredName'].'\',\''.$disaster['insuredFname'].'\',\''.$disaster['vehicle'].'\',\''.$disaster['disasterDate'].'\')"><br>');
                                    }
                                echo "</div>";
                            echo "</div>";
                        }
                    } else {
                        echo '<h4 class="faq-item">Aucun sinistre touvé.</h4>';
                    }
                }
        ...
        <script>
            ...
            function validateDisaster(name, fname, vehicle, disasterDate){
                ...
                xhttp.send('disaster=ok&name='+name+'&fname='+fname+'&vehicle='+vehicle+'&disasterDate='+disasterDate);
            }
        </script>
    </main>
